...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validation - only check required fields for our simplified forms
    ...
    // Generate unique email to avoid conflicts since we don't collect real emails
    const timestamp = Date.now();
    const uniqueEmail = email || `student_${timestamp}@${course.toLowerCase().replace(/\s+/g, '')}.local`;
